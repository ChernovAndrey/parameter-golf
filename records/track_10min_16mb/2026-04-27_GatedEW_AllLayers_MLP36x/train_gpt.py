...
class GPT(nn.Module):
	def __init__(self,h):
		...
		# Apply Qwen G1 gated attention to every layer when GATED_ATTN_ENABLED=1
		# (default). With elementwise gating this adds 11 x dim*dim = 11 x 262K
		# params (~+1.0 MB after int6 GPTQ + Brotli). The default MLP_MULT=3.6
		# (vs SOTA's 4.0) reclaims ~1.13 MB; predicted artifact ~15.89 MB
		# (~110 KB headroom). Lower MLP_MULT to 3.5 if a run lands over budget.
		gated_mode=h.gated_attn_mode if h.gated_attn_enabled else None
		self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,gated_mode=gated_mode)for i in range(h.num_layers)])
		if h.rope_dims>0:
