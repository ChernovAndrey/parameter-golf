...
class GPT(nn.Module):
	def __init__(self,h):
		...
		# Qwen G1 gating and MLP 3.25x (PARALLEL_MLP_MULT). Hypothesis: the gate
		# substitutes for MLP capacity precisely where attn output enters the residual
		# without a downstream MLP nonlinearity, which is the parallel-residual zone.
		# h=1664 = 13x128 so all GPTQ blocks divide cleanly. Per-token MLP volume in
		# this zone (4 x h=1664 = 6656) closely matches FatBlock's validated 6144.
		# Predicted artifact ~15.79 MB (~210 KB headroom).
		blocks_list=[]
