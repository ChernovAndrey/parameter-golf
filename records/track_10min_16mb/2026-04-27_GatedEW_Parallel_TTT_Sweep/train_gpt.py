...
class CausalSelfAttention(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len,gated_mode=None):
		super().__init__()
		...
		# Qwen G1 gated attention (NeurIPS 2025 best paper, arXiv 2505.06708):
		#   y = sigmoid(W_g x) * Attn(x), then proj. Elementwise gate is W_g: dim->dim;
		#   headwise is W_g: dim->num_heads (broadcast across head_dim). The gate adds
		#   per-token nonlinearity to the attention output (an "MLP-like" gate inside
		#   attention) and was strictly best at elementwise in the FatBlock sweep.
		self.gated_mode=gated_mode
		if gated_mode=='headwise':self.c_g=CastedLinear(dim,num_heads,bias=False)
		elif gated_mode=='elementwise':self.c_g=CastedLinear(dim,dim,bias=False)
		elif gated_mode in(None,'none'):self.c_g=None;self.gated_mode=None
		else:raise ValueError(f"Unknown gated_mode: {gated_mode!r}")
	...
	def forward(self,x):
		...
		if self.gated_mode=='headwise':gate=torch.sigmoid(self.c_g(x)).to(dtype=y.dtype);y=y*gate.unsqueeze(-1)
		y=y.reshape(bsz,seqlen,dim)
		if self.gated_mode=='elementwise':gate=torch.sigmoid(self.c_g(x)).to(dtype=y.dtype);y=y*gate
		return self.proj(y)
class MLP(nn.Module):
...
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,mlp_mult,rope_base,qk_gain_init,train_seq_len,layer_idx=0,ln_scale=False,gated_mode=None):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm();self.attn=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len,gated_mode=gated_mode);self.mlp=MLP(dim,mlp_mult);self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		# Targeted gated_ew + targeted MLP shrink, restricted to the parallel-residual
		# zone (layers >= PARALLEL_RESIDUAL_START, default 7). Sequential layers 0-6
		# keep SOTA's MLP 4.0x and have NO gate; parallel layers 7-10 use elementwise
		# Qwen G1 gating and MLP 3.4x (PARALLEL_MLP_MULT, h=1740). This pushes the
		# parallel-MLP capacity to the maximum that fits under the 16 MB cap when
		# combined with the 4 elementwise gates and Legal Score-First TTT (which is
		# eval-time only, zero artifact cost). Predicted artifact ~15.96 MB
		# (~40 KB headroom — comparable to SOTA's ~7 KB headroom level).
		# Per-token MLP volume in the parallel zone: 4 x 1740 = 6960
		# (vs FatBlock's validated 6144, ~13% wider).
		# Fallback: PARALLEL_MLP_MULT=3.375 (h=1728) for ~70 KB headroom.
		blocks_list=[]
		for i in range(h.num_layers):
			is_parallel=(h.parallel_residual_start>=0 and i>=h.parallel_residual_start)
			block_mlp_mult=h.parallel_mlp_mult if is_parallel else h.mlp_mult
			block_gated_mode=h.gated_attn_mode if(h.gated_attn_enabled and is_parallel)else None
			blocks_list.append(Block(h.model_dim,h.num_heads,h.num_kv_heads,block_mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,gated_mode=block_gated_mode))
