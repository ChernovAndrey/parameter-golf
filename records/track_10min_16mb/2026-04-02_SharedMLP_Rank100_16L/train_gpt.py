...
class Hyperparameters:
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 16))  # XSA on ALL layers
    ...
    ve_layers = os.environ.get("VE_LAYERS", "14,15")
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        if self.mlp_shared_up is not None:
            for g in range(self.num_shared_mlps):
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int,
                       layer_to_group: list[int] | None = None) -> dict[str, Tensor]:
    """Convert 3D bank tensors into individual 2D tensors with standard names.
    If shared MLP bases + adapters present, materializes effective weights."""
    out: dict[str, Tensor] = {}
    n = num_layers
    consumed = set()
...
def _unbank_for_quantization(sd: dict[str, Tensor], num_layers: int,
                              layer_to_group: list[int]) -> dict[str, Tensor]:
    """Convert state dict to quantization format: shared bases + individual adapters."""
    out: dict[str, Tensor] = {}
    n = num_layers
    consumed = set()
    for name, tensor in sd.items():
        if name == "qo_bank":
            ...
            for i in range(n):
                ...
                out[f"blocks.{i}.attn.c_v.weight"] = tensor[n + i]
            consumed.add(name)
    if "mlp_shared_up" in sd:
        num_shared = sd["mlp_shared_up"].shape[0]
        ...
        for i in range(n):
            ...
            out[f"blocks.{i}.mlp.proj.weight"] = sd["mlp_down_bank"][i]
        consumed.update({"mlp_up_bank", "mlp_down_bank"})
    for name, tensor in sd.items():
        if name not in consumed:
            out[name] = tensor
    return out

def _aggregate_hessians_for_shared(hessians: dict[str, Tensor], num_layers: int,
                                    layer_to_group: list[int], num_shared: int) -> dict[str, Tensor]:
    """Create aggregated Hessians for shared MLP bases from per-layer Hessians."""
    agg: dict[str, Tensor] = {}
    for g in range(num_shared):
        group_layers = [i for i in range(num_layers) if layer_to_group[i] == g]
        up_hs = [hessians[f"blocks.{i}.mlp.fc.weight"] for i in group_layers
                 if f"blocks.{i}.mlp.fc.weight" in hessians]
        if up_hs:
            agg[f"shared_mlp.{g}.up"] = sum(up_hs) / len(up_hs)
        dn_hs = [hessians[f"blocks.{i}.mlp.proj.weight"] for i in group_layers
                 if f"blocks.{i}.mlp.proj.weight" in hessians]
        if dn_hs:
            agg[f"shared_mlp.{g}.down"] = sum(dn_hs) / len(dn_hs)
    return agg

def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int,
                       template_sd: dict[str, Tensor],
                       layer_to_group: list[int] | None = None) -> dict[str, Tensor]:
    """Convert individual 2D tensors back into 3D bank tensors.
    Handles both standard unbanked format and shared+adapter format."""
    out: dict[str, Tensor] = {}
    n = num_layers
    has_shared = any(k.startswith("shared_mlp.") for k in sd)
    qo_slices = [None] * (2 * n)
    kv_slices = [None] * (2 * n)
    up_slices = [None] * n
    down_slices = [None] * n
...
def main() -> None:
    ...
    layer_to_group = base_model.layer_to_group if hasattr(base_model, 'layer_to_group') and base_model.layer_to_group else None
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_layers, layer_to_group=layer_to_group)
    ...
    # Quantize: shared+adapter format if adapters are present, else standard
    if layer_to_group:
        quant_sd = _unbank_for_quantization(sd_cpu, args.num_layers, layer_to_group)
        shared_hessians = _aggregate_hessians_for_shared(
            hessians, args.num_layers, layer_to_group, args.num_shared_mlps)
        hessians.update(shared_hessians)
        quant_result, quant_meta = mixed_quantize_int6(quant_sd, {"mlp", "attn", "adapter"}, hessians=hessians)
    else:
        quant_sd = unbanked_sd
